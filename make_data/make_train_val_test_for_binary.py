...
class SplitTool(object):
    ...
    def split_from_emp_syb(root_dir, datasets):
        ...
        for db in datasets:
            ...
            image_ibg_dir = os.path.join(root_dir, db, "{}_image_ibg".format(db))
            ...
            image_ibg_path = [os.path.join(image_ibg_dir, "{}_image_color_{}.png".format(db, i)) for i in
                              range(1, im_amounts + 1)]
            image_ibg_gray_path = [os.path.join(image_ibg_gray_dir, "{}_image_color_{}.png".format(db, i)) for i in
                                   range(1, im_amounts + 1)]
            image_ibg_hsv_path = [os.path.join(image_ibg_hsv_dir, "{}_image_color_{}.png".format(db, i)) for i in
                                  range(1, im_amounts + 1)]
            image_ibg_inverse_path = [os.path.join(image_ibg_inverse_dir, "{}_image_color_{}.png".format(db, i)) for i
                                      in
                                      range(1, im_amounts + 1)]
            # labels paths
            lb_paths = []
            ...
            for i in range(im_amounts):
                data.append([image_ibg_path[i], lb_paths[i]])
            for i in range(im_amounts):
                data.append([image_ibg_gray_path[i], lb_paths[i]])
            for i in range(im_amounts):
                data.append([image_ibg_hsv_path[i], lb_paths[i]])
            for i in range(im_amounts):
                data.append([image_ibg_inverse_path[i], lb_paths[i]])
            # split
